...
namespace Pillow
{
    ...
    inline template<typename T, typename U, typename = std::enable_if_t<std::is_arithmetic_v<T> && std::is_arithmetic_v<U>>>
    bool operator==(const Vector3<T>& a, const Vector2<U>& b)
    {
